...
//usará o arquivo falta.txt que contem o numero de quem faltou na prova
//
uses tardenoite;
var
//arquivos
...
//outras
 vet:vetor;
 a,b,c,posiv,np,ii,tot,tc,tt:integer;     //POSIV= INDICE DO VETOR
 numa,num:string[4];
Begin
 assign(arq,'cand.ind');
 assign(falta,'falta.txt');
 reset(arq);
 reset(falta);
...
//leitura dos faltantes
 ...
 repeat
  tt:=tt+1;
  readln(falta,numa);
  pebin1(vet,numa,tot,posiv);  //Procura NUMA em VET com TOT elementos - pesq binaria - se 
	                             //achou, devolve 
                               //em qual posicao de VET encontrou ou zero se nao 
															 //encontrou(POSIV) 
  ...
  read(arq,reg);
//ATUALIZA O CAMPO REG.FALTA COM 1 PARA INDICAR QUE O CANDIDATO FALTOU
  REG.FALTA:=1;
  IF(TT MOD 20 = 0)THEN
   BEGIN
